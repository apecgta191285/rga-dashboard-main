...
export function GlobalDateRangePicker() {
    ...
    return (
        <div className="bg-slate-100/50 p-0.5 rounded-md flex items-center border border-slate-200/60">
            {availableOptions.map((option) => (
                <button
                    key={option}
                    onClick={() => setDateRange(option as DateRangeOption)}
                    className={`px-2.5 py-1 text-[11px] font-medium rounded transition-all duration-200 ${dateRange === option
