import { Body, Controller, Get, Post, Delete, Query, UseGuards, Request, Res, UseInterceptors } from '@nestjs/common';
...
import { DashboardAiChatRequestDto, DashboardAiChatResponseDto } from './dto/dashboard-ai-chat.dto';
...
export class DashboardController {
  ...
  @Post('ai-chat')
  @ApiOperation({
    summary: 'Ask AI assistant using tenant-scoped data from metrics and campaigns tables',
  })
  @ApiBody({ type: DashboardAiChatRequestDto })
  @ApiResponse({
    status: 200,
    description: 'AI assistant response generated from DB data',
    type: DashboardAiChatResponseDto,
  })
  async aiChat(
    @CurrentUser() user: any,
    @Body() body: DashboardAiChatRequestDto,
  ): Promise<DashboardAiChatResponseDto> {
    return this.dashboardService.aiChat(user, body);
  }
