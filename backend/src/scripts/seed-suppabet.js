...
function generateMatchData(tenantId, date, index) {
    ...
    const sportTeams = teamNames[sportType];
    ...
    if (status === 'completed') {
        if (result === 'home') {
            payout = totalAmount * 0.3 * homeOdds;
        }
        else if (result === 'away') {
            payout = totalAmount * 0.3 * awayOdds;
        }
        else {
            payout = totalAmount * 0.2 * drawOdds;
        }
...
seedSuppabet();
//# sourceMappingURL=seed-suppabet.js.map